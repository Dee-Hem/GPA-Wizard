...
export default function TimetablePage() {
  ...
  // THE SYNC LOGIC: Using Local Notifications instead of Calendar
  const syncWithNotifications = async () => {
    try {
      const perms = await LocalNotifications.requestPermissions();
      if (perms.display !== 'granted') {
        alert("Wizard needs notification permission to remind you of classes!");
        return;
      }
      ...
      const notifications = courses.map((course, index) => {
        // Simple logic: Send a test notification in 5 seconds to confirm it works
        // In a real build, we'd calculate the 'at' date based on course.day and course.time
        return {
          title: `Class Reminder: ${course.code}`,
      ...
      if (notifications.length > 0) {
        ...
        alert("Success! The Wizard will notify you before your classes.");
      } else {
        alert("Add some classes first, baba!");
      }
    } catch (err) {
      alert("Sync failed. Ensure @capacitor/local-notifications is in package.json");
    }
  };
  ...
  return (
    <div className="p-4 pb-24 max-w-2xl mx-auto space-y-6 min-h-screen bg-background">
      <div className="flex justify-between items-center pt-4">
        <div>
          <h1 className="text-3xl font-black text-foreground tracking-tight">Timetable</h1>
          <p className="text-muted-foreground text-sm">Never miss a lecture</p>
        </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar border-b border-border/50">
        {days.map((day) => (
          <button
            ...
            onClick={() => setActiveDay(day)}
            className={`px-5 py-2 rounded-xl text-sm font-bold whitespace-nowrap transition-all ${
              activeDay === day ? 'bg-primary text-primary-foreground shadow-md' : 'bg-card text-muted-foreground border border-border'
            }`}
          >
            {day}
          </button>
      ...
      {showAddForm && (
        <Card className="border-2 border-primary/20 bg-card animate-in fade-in slide-in-from-top-4">
          <CardContent className="p-4 space-y-3">
            ...
            />
            <div className="flex gap-2">
              <input type="time" className="flex-1 p-3 rounded-lg bg-background border border-border text-sm" value={newCourse.time} onChange={(e) => setNewCourse({...newCourse, time: e.target.value})} />
              <input placeholder="Venue" className="flex-1 p-3 rounded-lg bg-background border border-border text-sm" value={newCourse.location} onChange={(e) => setNewCourse({...newCourse, location: e.target.value})} />
            </div>
            <Button onClick={addCourse} className="w-full font-bold">Save to {activeDay}</Button>
          </CardContent>
        </Card>
      ...
      <div className="fixed bottom-6 right-6 left-6">
         <Button onClick={syncWithNotifications} className="w-full h-14 shadow-2xl gap-2 rounded-2xl font-black bg-primary text-primary-foreground hover:scale-[1.02] active:scale-[0.98] transition-all">
